/**
 * GET /api/paysync/cycles/[id] -- single billing cycle detail
 *
...
import { handleGetCycle } from "@infinityrx/module-paysync/bff";
...
export async function GET(
  ...
  if (!id) {
    return NextResponse.json(
      { error: { code: "MISSING_ID", message: "Cycle id is required" } },
  ...
  try {
    const result = await handleGetCycle(id, bffReq, client);
    ...
    return NextResponse.json(
      { error: { code: "UPSTREAM_ERROR", message: "Failed to fetch cycle" } },
