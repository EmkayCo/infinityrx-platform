"""Run the SAM.gov exclusions ingester against the live DB.

LOADER-07. Loads shared.sam_exclusions from the SAM.gov public API
(sam.gov/data-services/Exclusions/Public). The ingester source already
exists at shared/data_ingestion/sources/sam_exclusions.py — this is
just the CLI wrapper that matches the scripts/load_oig_leie.py shape.
...
async def main() -> None:
    ...
    print("=" * 60)
    print(f"SAM.gov: status={result.status}")
