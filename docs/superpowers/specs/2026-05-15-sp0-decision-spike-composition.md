...
**Status:** v5 drafted 2026-05-15. v4 was committed `beec4af`; codex pass-4 verified 2/4 findings CLOSED (yaml-comment header, stale symlink phrase), 2 PARTIALLY-CLOSED — the `import/no-restricted-paths` example used a single wildcard zone (`target: packages/modules/*/src/**` + `from: packages/modules/*/src/**`) that would also forbid same-module imports. v5 closes them by making the zone array EXPLICIT per-module pair: `target` is one module, `from` is one sibling module, generated by `scripts/generate-eslint-zones.ts` into `packages/shell/src/_generated/eslint-zones.ts` (with `input_hash` header per §5.3) and imported by the static `eslint.config.js` as `GENERATED_MODULE_ZONES`. For N modules the generator emits N*(N-1) explicit zones — none of which forbid same-module imports because `from` never names the target itself.

v4 closes (carried into v5 — no further changes needed): per-file-type `input_hash` header syntax; opening summary cleanup. v3 closes (carried into v5): pairwise collision surfaces broadened; secret validation PR-CI vs deploy/boot split; content-hash staleness; server-graph vs client-bundle audit split.
...
export default defineConfig({
  ...
  rules: {
    ...
    //
    // IMPORTANT: zones must be EXPLICIT per-module, NOT a single wildcard zone.
    // A wildcard `target: packages/modules/*/src/**` + `from: packages/modules/*/src/**`
    // would forbid same-module imports too. The correct shape is one zone per module
    // where `target` is THIS module and `from` is EVERY OTHER module (siblings only).
    // The array below is generated by `scripts/generate-eslint-zones.ts` at lint-setup
    // and imported here — the example shows the generated shape for a 3-module repo:
    "import/no-restricted-paths": ["error", {
      zones: GENERATED_MODULE_ZONES,   // imported from packages/shell/src/_generated/eslint-zones.ts
      // GENERATED_MODULE_ZONES expands to one entry per (module, sibling) pair, e.g.:
      //   [
      //     { target: "./packages/modules/reclaimrx/**", from: "./packages/modules/paysync/**",
      //       message: "reclaimrx must not import paysync. Use packages/contract." },
      //     { target: "./packages/modules/reclaimrx/**", from: "./packages/modules/billing/**",
      //       message: "reclaimrx must not import billing. Use packages/contract." },
      //     { target: "./packages/modules/paysync/**",   from: "./packages/modules/reclaimrx/**",
      //       message: "paysync must not import reclaimrx. Use packages/contract." },
      //     { target: "./packages/modules/paysync/**",   from: "./packages/modules/billing/**",
      //       message: "paysync must not import billing. Use packages/contract." },
      //     { target: "./packages/modules/billing/**",   from: "./packages/modules/reclaimrx/**",
      //       message: "billing must not import reclaimrx. Use packages/contract." },
      //     { target: "./packages/modules/billing/**",   from: "./packages/modules/paysync/**",
      //       message: "billing must not import paysync. Use packages/contract." },
      //   ]
      // For N modules this is N*(N-1) explicit zones. None of them forbid same-module
      // imports because `from` always names a sibling, never the target itself.
    }],
  },
});
```

`scripts/generate-eslint-zones.ts` emits `packages/shell/src/_generated/eslint-zones.ts` with the concrete array. The script reads every `packages/modules/<name>/module.config.ts` to discover module names, then for every (target, from) pair where `target != from` it emits a zone. The file carries an `// input_hash: sha256:<hex>` header per §5.3 — CI re-runs the generator and fails the build if the embedded hash drifts.

The same script also updates the static workspace `eslint.config.js` to `import { GENERATED_MODULE_ZONES } from "./packages/shell/src/_generated/eslint-zones"` — that import is the only line in `eslint.config.js` that changes when a new module is added.
