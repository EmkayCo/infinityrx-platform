...
| # | Issue | R3 decision |
|---|---|---|
| R3.1 | MFA gate as specified was non-sensical against HEAD. For MFA-required tenants, `modules/core-platform/src/auth/api/auth_router.py:152-216` issues the JWT only after `/mfa/verify` succeeds; for tenants with `mfa_required=False`, `auth_router.py:143` issues the JWT directly after password auth. Either way, HEAD's `CurrentUser` has no `mfa_verified` attribute, JWT claims have no MFA claim, sessions table has no `mfa_verified` column. The proposed `require_mfa_for_phi` would have nothing to read. | **DROP the per-route MFA dependency requirement.** For MFA-required tenants, MFA is enforced at the AUTH GATE: JWT issuance is blocked until MFA verification succeeds. For tenants with `mfa_required=False`, MFA is by definition not required for that tenant. Adding `require_mfa_for_phi` would attempt to duplicate this enforcement against an attribute that doesn't exist. Plan B's MFA "gate tests" are replaced with an AUTH-GATE assertion per new route: `request without Authorization header → 401`. Session-level MFA-reverification-on-PHI-access (e.g., "force fresh MFA within 15 min before PHI route access") is a core-platform-level concern — not paysync-specific — and is documented as out-of-scope for SP-1. Drops the MFA dependency file and the `require_mfa_for_phi` Task 3 sub-step (R2 had numbered it Task 3.1); removes "MFA gate" wording from integration test scope; keeps the upload router task itself intact. |
...
| 6 | MFA gate tests asserted "valid JWT with `mfa_verified=False` → 403" but no concrete dependency named. HEAD's `get_current_user` doesn't check `mfa_verified`; tenant carries `mfa_required` boolean. | R2 proposed introducing `require_mfa_for_phi` FastAPI dependency. **R3 SUPERSEDED this — see R3.1 above.** R3 drops the per-route MFA dependency entirely; MFA is enforced at JWT issuance for MFA-required tenants. R2's `require_mfa_for_phi` is NOT introduced. |
...
- `modules/billing/tests/integration/test_uploads_router.py` — 5 routes × 3 roles + per-endpoint cross-tenant + per-endpoint auth-gate (no-token → 401) + per-read-endpoint PHI audit (R3.1: auth-gate replaces R2's MFA gate)
- `modules/billing/tests/integration/test_inbox_router.py` — real items per kind + cross-tenant + auth-gate per endpoint (R3.1)
...
**Per-route MFA reverification is OUT OF SCOPE for SP-1.** HEAD's auth flow has two paths: for MFA-required tenants, `modules/core-platform/src/auth/api/auth_router.py:152-216` issues the JWT only at `/auth/mfa/verify` success. For tenants with `mfa_required=False`, `auth_router.py:143` issues the JWT directly after password auth (MFA not required for that tenant by configuration). Adding a per-route `require_mfa_for_phi` dependency would attempt to read an attribute (`mfa_verified` on CurrentUser / JWT claims / sessions) that does not exist in HEAD.

Plan B's new routes use the standard `Depends(get_current_user)` dependency. Test assertion per route: "returns 401 without bearer token" — this proves auth-gate protection. For MFA-required tenants (`tenant.mfa_required=True`), JWT issuance is blocked at the auth gate until `/auth/mfa/verify` succeeds, so any valid JWT from such a tenant is implicitly MFA-verified. Tenants with `mfa_required=False` opt out of MFA entirely by configuration and receive JWTs after password auth alone. Either way, no `mfa_verified=False` JWT can exist in HEAD because no such attribute exists on CurrentUser / JWT claims / sessions, so a "403 on mfa_verified=False" test cannot be written.
...
**Auth-gate test** per new route (replaces R2's MFA gate test): `GET/POST <new route>` without Authorization header → 401. This proves auth-gate protection. For MFA-required tenants, JWT issuance is blocked until MFA verification succeeds (per `modules/core-platform/src/auth/api/auth_router.py:152-216`), so any valid JWT from an MFA-required tenant is necessarily MFA-verified. Tenants with `mfa_required=False` opt out of MFA entirely by configuration.
...
- [ ] Auth-gate test per new route: request without Authorization header returns 401 (R3.1: replaces the MFA-gate test; for MFA-required tenants, JWT issuance is blocked until MFA verification succeeds per modules/core-platform/src/auth/api/auth_router.py:152-216; tenants with `mfa_required=False` opt out of MFA entirely; either way, no `mfa_verified=False` JWT can exist in HEAD because no such attribute exists)
...
- SP-0 auth/JWT infrastructure: `shared.auth.dependencies.get_current_user` for current user. For MFA-required tenants, MFA is enforced upstream at JWT issuance (`modules/core-platform/src/auth/api/auth_router.py:152-216` blocks JWT issuance until `/mfa/verify` succeeds); for tenants with `mfa_required=False`, JWT is issued directly after password auth (`auth_router.py:143`). No per-route MFA dependency is used or needed (R3.1).
