"""Integration test: full middleware stack is mounted in member-management create_app().
...
def test_hsts_header_present_on_health_response() -> None:
    ...
    assert response.headers.get("Strict-Transport-Security") is not None, (
        "SecurityHeadersMiddleware not mounted in member-management create_app() — LESSON-006"
    )
