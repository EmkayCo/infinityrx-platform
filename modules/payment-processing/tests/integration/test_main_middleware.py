"""Integration test: SecurityHeadersMiddleware is mounted in payment-processing create_app().
...
def test_hsts_header_present_on_health_response() -> None:
    ...
    assert response.headers.get("Strict-Transport-Security") is not None, (
        "SecurityHeadersMiddleware not mounted in payment-processing create_app() — LESSON-006"
    )
