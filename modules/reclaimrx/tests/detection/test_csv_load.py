...
class TestInsertTupleBuilder:
    """Unit tests for _build_insert_tuple — the pure tuple-builder used by the
    PostgreSQL execute_values bulk INSERT path.  No DB required.
    ...
    row_data must be wrapped in psycopg2.extras.Json so psycopg2 passes it
    as a JSONB-typed parameter rather than a plain string.
    """

    def test_build_insert_tuple_has_correct_length(self):
        """Tuple produced by _build_insert_tuple has exactly 11 elements
        matching _INSERT_COLUMNS order.
        """
        from src.detection.csv_ingest import _build_insert_tuple
    ...
    def test_build_insert_tuple_column_order(self):
        """Elements appear in _INSERT_COLUMNS order:
        (id, tenant_id, detection_run_id, row_number, row_data,
         resolved_pharmacy_npi, resolved_ndc, resolved_client_id,
         resolved_program_id, resolution_method, resolution_notes).
        """
        ...
        assert t[0] == str(row_id)        # id
        assert t[1] == str(tenant_id)      # tenant_id
        assert t[2] == str(run_id)         # detection_run_id
        assert t[3] == 42                  # row_number
        # t[4] is the Json-wrapped row_data — checked in next test
        assert t[5] == "1234567890"        # resolved_pharmacy_npi
        assert t[6] == "12345678901"       # resolved_ndc
        assert t[7] is None               # resolved_client_id
        assert t[8] is None               # resolved_program_id
        assert t[9] == "declared"          # resolution_method
        assert t[10] is None              # resolution_notes

    def test_build_insert_tuple_row_data_is_json_adapted(self):
        """row_data element (index 4) is a psycopg2.extras.Json instance wrapping
        the original dict so psycopg2 passes it as JSONB, not a plain string.
        """
        ...
        assert isinstance(t[4], Json)
        # Confirm the wrapped dict is the original row_data
        assert t[4].adapted == row_data

    def test_build_insert_tuple_null_columns_are_none(self):
        """NULL columns (resolved_client_id, resolved_program_id, resolution_notes)
        appear as Python None so execute_values passes them as SQL NULL.
        """
        from src.detection.csv_ingest import _build_insert_tuple
    ...
    def test_build_insert_tuple_resolution_notes_preserved(self):
        """resolution_notes string value appears at index 10."""
        from src.detection.csv_ingest import _build_insert_tuple
