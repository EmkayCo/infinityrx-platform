...
RUN rule count: 13 (MFR-008 reclassified to DEFERRED in dry-run fix — needs
statement-only-pharmacy reference data not present in the CSV).
...
RULE_TYPE_CATALOG: list[dict] = [
    ...
    {"code":'MFR-008',"name":'Statement Credit Abuse',"description":'Statement account pharmacy submitting claims that should be statement-only',"family":_CATEGORY_FAMILY['billing_pattern'],"parameter_schema_version":"1.0","default_severity":'critical',"default_confidence":0.9,"requires_baseline":False,"requires_history":False,"deferred_data_feed":True,"deferred_reason":'Needs statement-only-pharmacy reference data (which pharmacies are contractually statement-only); a populated statement_account is normal, not fraud. No such reference feed in the CSV.',"required_data_columns":[],"default_parameters":{'pattern': 'statement_credit_abuse'}},
    ...
    {"code":'TH-002',"name":'Telehealth Geographic Dispersion',"description":'Telehealth prescriber patients spread across more than 10 states',"family":_CATEGORY_FAMILY['billing_pattern'],"parameter_schema_version":"1.0","default_severity":'medium',"default_confidence":0.7,"requires_baseline":False,"requires_history":True,"deferred_data_feed":False,"deferred_reason":'',"required_data_columns":['prescriber_npi', 'patient_state'],"default_parameters":{'field': 'patient_state_count', 'operator': 'gt', 'threshold': 10, 'min_claims': 20}},
    ...
    {"code":'TH-005',"name":'Telehealth Prescriber-Pharmacy Affinity',"description":'More than 50% of telehealth prescriber scripts filled at single pharmacy',"family":_CATEGORY_FAMILY['billing_pattern'],"parameter_schema_version":"1.0","default_severity":'high',"default_confidence":0.5,"requires_baseline":False,"requires_history":True,"deferred_data_feed":False,"deferred_reason":'',"required_data_columns":['prescriber_npi', 'pharmacy_npi'],"default_parameters":{'field': 'top_pharmacy_share', 'operator': 'gt', 'threshold': 0.5, 'min_claims': 20}},
    {"code":'REJECT-75-70',"name":'Reject-75->70 Fast Rebill',"description":'PA-required reject (75) followed by not-covered reject (70) for same Rx within 24h -- group key rx_number_hash (locked decision #1)',"family":_CATEGORY_FAMILY['billing_pattern'],"parameter_schema_version":"1.0","default_severity":'high',"default_confidence":0.8,"requires_baseline":False,"requires_history":True,"deferred_data_feed":False,"deferred_reason":'',"required_data_columns":['rx_number_hash', 'reject_code', 'date_added_timestamp'],"default_parameters":{'bucket_le12h_enabled': True, 'bucket_12_24h_enabled': True}}
]
