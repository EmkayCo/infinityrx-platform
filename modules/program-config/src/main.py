"""Program Config FastAPI application.
...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    """Startup: install slow-query logger if available."""
    try:
...
def create_app() -> FastAPI:
    ...
    app = FastAPI(
        lifespan=lifespan,
        title="InfinityRx Program Configuration & Onboarding",
        version="1.0.0",
        description="Program onboarding, BRD template engine, contract management, manufacturer self-service.",
    ...
    async def health() -> dict:
        return {"status": "ok", "module": "program-config"}
