"""Adjudication engine FastAPI application entry point.
...
logger = logging.getLogger("adjudication_engine.main")
...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    try:
        ...
        logger.exception("adjudication_engine.consumer_wiring_failed")
...
def create_app() -> FastAPI:
    ...
    app = FastAPI(
        lifespan=lifespan,
        title="InfinityRx Adjudication Engine",
        version="1.0.0",
        description="Real-time pharmacy claim adjudication — NCPDP D.0, DUR, COB, accumulator detection",
    ...
    async def health() -> dict:
        return {"status": "healthy", "module": "adjudication-engine"}
