"""Integration tests for Reconciliations GET router (SP-1 Plan C Task B2).
...
class TestReconciliationsAuthGate:
    ...
    def test_get_by_id_no_auth_returns_401(self, _client):
        resp = _client.get(
            f"/api/v1/billing/reconciliations/{uuid.uuid4()}",
            headers={"X-Tenant-Id": TENANT_A},
        )
        assert resp.status_code == 401


class TestReconciliationsTenantMismatch:
...
class TestReconciliationsGetById:
