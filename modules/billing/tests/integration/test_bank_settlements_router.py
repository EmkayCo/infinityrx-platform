"""Integration tests for Bank Settlements GET router (SP-1 Plan C Task B2).

Covers:
  - Auth gate (no token -> 401) on both endpoints
  - Tenant header mismatch (X-Tenant-Id != JWT tenant) -> 403
  - GET / returns 200 + bare array (empty -- stub returns empty)
...
class TestBankSettlementsAuthGate:
    def test_list_no_auth_returns_401(self, _client):
        resp = _client.get(
            "/api/v1/billing/bank-settlements",
    ...
    def test_get_by_id_no_auth_returns_401(self, _client):
        resp = _client.get(
            f"/api/v1/billing/bank-settlements/{uuid.uuid4()}",
            headers={"X-Tenant-Id": TENANT_A},
        )
        assert resp.status_code == 401


class TestBankSettlementsTenantMismatch:
...
class TestBankSettlementsGetById:
    ...
    def test_get_cache_control_on_404(self, _client):
        from shared.auth.dependencies import get_current_user
        from src.main import app
