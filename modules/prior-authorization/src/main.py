"""Prior Authorization FastAPI application.
...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    """Startup: install slow-query logger."""
    try:
...
def create_app() -> FastAPI:
    ...
    app = FastAPI(
        lifespan=lifespan,
        title="InfinityRx Prior Authorization",
        version="1.0.0",
        description="PA submission, criteria evaluation, decisions, appeals, ePA, and copay first-fill.",
    ...
    async def health() -> dict:
        return {"status": "ok", "module": "prior-authorization"}
