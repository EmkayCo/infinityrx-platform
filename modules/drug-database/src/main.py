...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    """Startup: install slow-query logger on the sync engine (M-04) and
    configure auth so Depends(get_current_user) can resolve incoming JWTs."""
    try:
...
def create_app() -> FastAPI:
    ...
    # B11 F-002: root /health route (the existing /health on the
    # /api/v1/drugs router lived at /api/v1/drugs/health, invisible to
    # smoke probes that hit root). Matches the convention used by
    # core-platform, billing, reclaimrx, prescriber-directory.
    @app.get("/health")
    async def health() -> dict:  # pragma: no cover — exercised by tests/test_main.py
        return {"status": "ok", "service": "drug-database"}
