"""B9.B C11 — Tier A batch 06 contract tests.

Mirrors batch_04 test pattern exactly. Coverage shape per spec:

  test_spec_columns_match_ddl_manifest       — column-set matches DDL
  test_spec_nullable_set_matches_manifest    — nullable frozenset subset of DDL nullable
  test_delta_semantics_acd_cycle_for_each_spec — A/C/D/re-A correctness
  test_spec_is_tagged_tier_a                 — tier=Tier.A invariant
  test_spec_loader_group_is_fdb_tier_a       — loader_group invariant
  test_spec_record_counts_key_set            — record_counts_key set
  test_batch_06_has_expected_table_count     — exactly 10 specs
"""
...
from drug_database.services.fdb_tier_a.batch_06 import SPECS
...
def test_delta_semantics_acd_cycle_for_each_spec(spec, sqlite_engine) -> None:
    """Every batch_06 spec passes the A/C/D/re-A correctness contract.

    Each Tier A table is UPSERT_BY_NATURAL_KEY: id+desc semantics. The
    natural key is the FIRST column; the second column is the
    description (or the first non-key column for multi-column tables).
    C-step changes the description; D-step deletes; re-A restores.
    Correct UPSERT (ON CONFLICT DO UPDATE) passes all four sub-assertions
    in _fdb_contract.

    For multi-column specs (>2 cols), the simulator's UPSERT update_cols
    dict keys into the supplied row for every non-natural-key column, so
    all columns must be present. Columns beyond col[1] receive a stable
    placeholder value ("placeholder_N") so the simulator can build the
    full UPDATE SET clause without a KeyError.
    """
    assert spec.delta_semantics is DeltaSemantics.UPSERT_BY_NATURAL_KEY, (
        f"{spec.table_name}: batch_06 invariant — all specs are "
    ...
    # Build placeholder values for any extra non-key columns (col index >= 2).
    extra_cols = {
...
def test_batch_06_has_expected_table_count() -> None:
    """Batch 06 ships 10 specs."""
    assert len(SPECS) == 10, (
        f"batch_06 SPECS has {len(SPECS)} entries; expected 10. "
        f"If extending this batch beyond 10 tables, also extend "
        f"this assertion (or move overflow to batch_07)."
    )
