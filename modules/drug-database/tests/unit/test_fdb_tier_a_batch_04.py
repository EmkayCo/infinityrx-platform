"""B9.B C9 — Tier A batch 04 contract tests.

Mirrors batch_01 test pattern exactly. Coverage shape per spec:
  ...
  test_batch_04_has_expected_table_count     — exactly 10 specs
"""
...
from drug_database.services.fdb_tier_a.batch_04 import SPECS
...
def test_delta_semantics_acd_cycle_for_each_spec(spec, sqlite_engine) -> None:
    """Every batch_04 spec passes the A/C/D/re-A correctness contract.
    ...
    assert spec.delta_semantics is DeltaSemantics.UPSERT_BY_NATURAL_KEY, (
        f"{spec.table_name}: batch_04 invariant — all specs are "
...
def test_batch_04_has_expected_table_count() -> None:
    """Batch 04 ships 10 specs."""
    assert len(SPECS) == 10, (
        f"batch_04 SPECS has {len(SPECS)} entries; expected 10. "
        f"If extending this batch beyond 10 tables, also extend "
        f"this assertion (or move overflow to batch_05)."
    )
