"""B9.B C10 — Tier A batch 05 contract tests.

Mirrors the canonical pattern in test_fdb_tier_a_batch_01.py. One
deviation: RPEIDM0_DOSAGE_FORM_MSTR (5-column table) uses columns[1]
as the ACD-cycle description column, same as all 2-column tables.
SQLite String columns are nullable by default so the simulator
tolerates partial row dicts without error.
...
Coverage shape (per spec, per test function):
  ...
  test_delta_semantics_acd_cycle_*          — A→C→D→re-A correct
  test_spec_is_tagged_tier_a                — tier invariant
  test_spec_loader_group_is_fdb_tier_a      — loader_group invariant
  test_spec_record_counts_key_set           — record_counts_key present
  test_batch_05_has_expected_table_count    — 10 specs exactly
"""
...
from drug_database.services.fdb_tier_a.batch_05 import SPECS
...
def test_delta_semantics_acd_cycle_for_each_spec(spec, sqlite_engine) -> None:
    """Every batch_05 spec passes the A/C/D/re-A correctness contract.

    Each Tier A table is UPSERT_BY_NATURAL_KEY: id+desc semantics. The
    natural key is the FIRST column; the second column is used as the
    assertion anchor (initial vs. changed). For tables with more than
    two columns, all non-key columns are included in initial_row and
    changed_values as required by the SQLite simulator's ON CONFLICT
    DO UPDATE set_ construction (it iterates all_columns).
    """
    assert spec.delta_semantics is DeltaSemantics.UPSERT_BY_NATURAL_KEY, (
        f"{spec.table_name}: batch_05 invariant — all specs are "
        f"UPSERT_BY_NATURAL_KEY (id+desc lookups). Got "
        f"{spec.delta_semantics}."
    )
    natural_key_col = spec.columns[0]
    desc_col = spec.columns[1]
    non_key_cols = [c for c in spec.columns if c != natural_key_col]
    table_name_safe = spec.table_name.lower()

    # Build initial and changed rows that include ALL non-key columns so
    # the simulator's ON CONFLICT DO UPDATE set_ dict can be constructed
    # without KeyError for 3-, 4-, and 5-column tables.
    initial_row = {c: ("initial description" if c == desc_col else f"init_{c}") for c in non_key_cols}
    changed_values = {c: ("CHANGED DESCRIPTION" if c == desc_col else f"changed_{c}") for c in non_key_cols}
...
def test_batch_05_has_expected_table_count() -> None:
    """Batch 05 ships 10 specs."""
    assert len(SPECS) == 10, (
        f"batch_05 SPECS has {len(SPECS)} entries; expected 10. "
        f"If extending this batch, also extend this assertion "
        f"(or move overflow to batch_06)."
    )
